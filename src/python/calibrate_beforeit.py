...
def run_calibration(model_wrapper: BeforeITModelWrapper,
                   target_data: np.ndarray,
                   results_folder: str,
                   max_iterations: int = 20,
                   ensemble_size: int = 4,
                   batch_size: int = 4,
                   loss_function: str = "msm") -> Tuple[np.ndarray, np.ndarray, Calibrator]:
    """
    ...
    batch_size : int
        Number of parameter sets per iteration
    loss_function : str
    ...
    # Set up samplers
    samplers = setup_samplers(batch_size)
    
    # Create model function with ensemble averaging
    def model_func(theta, N, rndSeed):
        return model_wrapper.run_simulation(theta, N, rndSeed, ensemble_size)
...
def plot_calibration_progress(calibrator: Calibrator, 
    ...
    losses_per_batch = [calibrator.losses_samp[batch_nums == i] for i in range(max_batch)]
    mins_per_batch = np.array([np.min(l) if len(l) > 0 else np.inf for l in losses_per_batch])
    cummin_per_batch = [np.min(mins_per_batch[:i+1]) for i in range(len(mins_per_batch))]
    ...
    plt.plot(range(len(cummin_per_batch)), cummin_per_batch, 'r-', linewidth=2, label='Best so far')
    ...
    for i in range(min(3, len(parameter_names))):
        plt.scatter(batch_nums, calibrator.params_samp[:, i], 
                   alpha=0.6, s=20, label=parameter_names[i])
    plt.xlabel('Iteration')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(results_folder, "calibration_progress.png"), dpi=300, bbox_inches='tight')
    plt.show()


def plot_parameter_exploration(calibrator: Calibrator, 
                              parameter_names: List[str], 
                              results_folder: str):
    ...
    for i in range(plot_params):
        for j in range(plot_params):
            ...
            if i == j:
                # Diagonal: histogram
                ax.hist(calibrator.params_samp[:, i], bins=20, alpha=0.7, color='skyblue')
                ax.set_title(parameter_names[i])
            else:
                # Off-diagonal: scatter plot
                scatter = ax.scatter(calibrator.params_samp[:, j], 
                ...
                ax.scatter(calibrator.params_samp[idxmin, j], 
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(results_folder, "parameter_exploration.png"), dpi=300, bbox_inches='tight')
    plt.show()


def compare_best_simulation(calibrator: Calibrator,
                           model_wrapper: BeforeITModelWrapper,
                           target_data: np.ndarray,
                           results_folder: str):
    ...
    simulated_data = model_wrapper.run_simulation(best_params, T, seed=42, ensemble_size=8)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(results_folder, "target_vs_simulation.png"), dpi=300, bbox_inches='tight')
    plt.show()
...
def save_detailed_results(calibrator: Calibrator,
                         model_wrapper: BeforeITModelWrapper,
                         target_data: np.ndarray,
...
def analyze_calibration_results(calibrator: Calibrator,
                               model_wrapper: BeforeITModelWrapper,
                               target_data: np.ndarray,
                               data_metadata: Dict[str, Any],
                               target_data_source: str,
    ...
    save_detailed_results(calibrator, model_wrapper, target_data, data_metadata, 
                         target_data_source, base_parameters, results_folder)
